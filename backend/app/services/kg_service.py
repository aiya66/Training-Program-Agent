...
"""
        ...
        包含以下章节（请使用Markdown格式），**请务必尽可能详细地展开每一部分，提供深入的分析和具体的例子，使报告内容充实、丰富、篇幅足够长**：

        1. **现状深度剖析**：
           - 基于生成的知识图谱，详细解读当前专业的课程结构与技能覆盖情况。
           - 结合用户上传的培养方案（如果有），深入分析其培养目标、毕业要求与实际教学设置的内在逻辑与潜在问题。
           - 请列举具体的课程名称或技能点作为论据，分析现有的优点和不足。

        2. **市场需求与差距分析**：
           - 详细对比当前的培养体系与互联网/重庆市高质量就业市场的实际需求（参考图谱数据摘要中的热门岗位和技能）。
           - 针对性地指出目前毕业生在核心技能（如编程语言、工具框架）、综合素质（如团队协作、创新思维）等方面存在的具体短板。
           - 引用图谱中的统计数据来支持你的观点。

        3. **全方位改进建议方案**（本部分是重点，请大幅展开）：
           - **课程体系重构**：不仅要建议增加什么课程，还要说明为什么要增加，以及建议的开设学期和教学重点。请提出至少3-5门具体的建议新增或调整课程。
           - **实践教学体系升级**：设计具体的实践项目或实训环节（如“AI+X”综合实训），描述项目的实施方式、技术栈和预期目标。
           - **产教融合深化路径**：提出具体的校企合作模式（如定制班、联合实验室）、实习基地建设方案或企业导师引入机制。
           - **教学方法与评价改革**：建议采用什么样的现代化教学手段（如项目式学习PBL、翻转课堂）以及多元化的评价方式。

        4. **预期成效与展望**：
           - 详细描述改进方案实施后，学生在就业竞争力、薪资水平、职业发展潜力等方面的具体提升预期。
           - 对该专业未来的发展方向进行展望，如何适应“人工智能+”时代的变革。
        
        **请注意：**
        - **输出内容越详细越好**，不要泛泛而谈，拒绝空洞的套话。
        - 每一个观点都要有理有据，充分利用提供的图谱数据和培养方案内容。
        - 保持专业、客观、有建设性的学术风格。
        """
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是资深的高校教育教学改革专家，擅长基于数据分析撰写详尽、深入、篇幅宏大的专业培养方案改进分析报告。"},
