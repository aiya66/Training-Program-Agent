...
const ProcessFlow = ({ currentStep = 0 }) => {
  ...
  const nodes = [
    ...
    { id: 'src-1', x: 20, y: 20, label: "海量岗位", icon: Briefcase, color: "blue", stepThreshold: 1, description: "采集全网海量招聘岗位数据" },
    { id: 'src-2', x: 20, y: 40, label: "高质量岗位", icon: Award, color: "blue", stepThreshold: 1, description: "筛选重点企业高质量岗位需求" },
    { id: 'src-3', x: 20, y: 60, label: "行业发展", icon: TrendingUp, color: "blue", stepThreshold: 1, description: "分析行业发展趋势报告" },
    { id: 'src-4', x: 20, y: 80, label: "政策文件", icon: FileText, color: "blue", stepThreshold: 1, description: "解析国家及地方相关政策文件" },

    // Step 2: Verification & Summary
    { id: 'verify', x: 35, y: 50, label: "验证汇总", icon: CheckSquare, color: "purple", stepThreshold: 2, description: "多源数据交叉验证与清洗汇总" },

    // Step 3: Graph Construction
    { id: 'build', x: 50, y: 50, label: "构建图谱", icon: Network, color: "orange", stepThreshold: 3, description: "基于汇总数据构建实体关系网络" },

    // Step 3.5: Graph Types (Parallel)
    { id: 'graph-1', x: 65, y: 30, label: "知识图谱", icon: Brain, color: "orange", stepThreshold: 3, description: "构建专业知识体系图谱" },
    { id: 'graph-2', x: 65, y: 50, label: "能力图谱", icon: Zap, color: "orange", stepThreshold: 3, description: "构建岗位核心能力图谱" },
    { id: 'graph-3', x: 65, y: 70, label: "素质图谱", icon: UserCheck, color: "orange", stepThreshold: 3, description: "构建综合素质要求图谱" },

    // Step 4: Display Page
    { id: 'end', x: 80, y: 50, label: "构建展示页面", icon: Layout, color: "green", stepThreshold: 4, description: "生成可视化交互展示页面" },

    // Step 5: Analysis & Suggestions (New)
    { id: 'analyze', x: 95, y: 50, label: "改进建议", icon: Lightbulb, color: "green", stepThreshold: 5, description: "生成培养方案分析与改进建议" },
  ];
